      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="float-end">
                <button type="button" id="addButton" class="btn btn-primary btn-sm">+Add New Status</button>
                </div>
                        ...
                        <div id="error-message1" class="alert alert-danger" style="display: none;"></div>
                            ...
                            <form id="dataForm">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="proj_status_name" name="proj_status_name" pattern="[a-zA-Z0-9\s]*" title="Special Characters not allowed" required>
                                </div>
                                   <div class="mb-3">
                                    <label for="name" class="form-label">Use As:</label>
                                    <input type="hidden" name="task_set_status" value="0">
                                    <input type="checkbox" name="task_set_status" id="task_set_status" value="1" checked>Task Status &nbsp;&nbsp;

                                    <input type="hidden" name="emp_set_status" value="0">
                                    <input type="checkbox" name="emp_set_status" id="emp_set_status" value="1">Employee Status &nbsp;&nbsp;
                                    
                                    <input type="hidden" name="proj_set_status" value="0">
                                    <input type="checkbox" name="proj_set_status" id="proj_set_status" value="1">Project Status &nbsp;&nbsp;

                                    <input type="hidden" name="ticket_set_status" value="0">
                                    <input type="checkbox" name="ticket_set_status" id="ticket_set_status" value="1">Ticket Status &nbsp;&nbsp;
                                </div>
...
$(document).ready(function () {
    $('#projectsTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('tasks.project_status') }}",
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex' },
                    { data: 'proj_status_name', name: 'proj_status_name' },
...
$('#addButton').click(function () {
  ...
  myModal.show();
  $('#dataModalLabel').text('Add New Status');
...
$(document).off('click', '.editButton').on('click', '.editButton', function () {
  ...
  $.get('tasks/edit_proj_status/' + id , function (data) {
      $('#dataModalLabel').text('Edit Status');
      $('#recordId').val(data.id);
      $('#proj_status_name').val(data.proj_status_name);
      $('input[name="task_set_status"]').prop('checked', data.task_set_status == 1);
      $('input[name="emp_set_status"]').prop('checked', data.emp_set_status == 1);
      $('input[name="proj_set_status"]').prop('checked', data.proj_set_status == 1);
  });
});
    ...
    $("#dataForm").validate({
        ...
        messages: {
            proj_status_name: {
                ...
                pattern: "Special characters are not allowed in the Status name.",
            },
        ...
        submitHandler: function (form) {
            ...
            const url = $('#recordId').val() ? '/update_proj_status/' + $('#recordId').val() : '/store_proj_status';
...
const id = $(this).data('id');
const url = '/tasks/' + id + '/destroy_project_status';
