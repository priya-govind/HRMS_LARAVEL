...
@endphp
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="content-wrapper">
            ...
            <div class="card-header">
                ...
                <div class="float-end">
                    <button type="button" id="addexpenseButton" class="btn btn-primary btn-sm">+Add New Transactions
                </div>
                    <div class="card-body">
    ...
    <hr>
            ...
            {{-- <pre>{{ print_r(request()->all(), true) }}</pre> --}}
             <form name="filter_expenses" method="get" action="{{ route('monthly_expense.index') }}" id="filter_expenses">
                ...
                <div class="row g-3">
                        ...
                        <div class="col-md-2">
                            <label for="work_mode" class="form-label">Expense Items</label>
                            <select class="form-select" id="exp_items" name="exp_items">
                                <option value="">Select</option>
                                @foreach($expenseItems as $item)
                                    <option value="{{ $item->id }}" {{ request('exp_items') == $item->id  ? 'selected' : '' }}>{{ $item->expense_type_name }}</option>
                                @endforeach
                            </select>
                        </div>
                                    ...
                                    <tr>
                                            ...
                                            @if($txn->transaction_type === 'debit')
                                                <ul>
                                                    @foreach($txn->items as $item)
                                                        <li>{{ $item->expenseItem->expense_type_name }}  - {{ $item->exp_amount }} </li>
                                                    @endforeach
                                                </ul>
                                        ...
                                        <td> 
                                            ...
                                            @if(!empty($txn->bill_refer))
                                                <a class="btn btn-info" id="viewImageBtn" data-id="{{ $txn->id }}"  data-image="/monthly_expenses/{{ $txn->bill_refer }}"><i class="fa-regular fa-image" ></i></a> |  
                                            @endif
                                            @if(($txn->last_entry==1))
                                            ...
                                            @endif
                                               @if($txn->transaction_type === 'debit' &&  \Carbon\Carbon::parse($txn->transaction_date)->isSameDay(\Carbon\Carbon::today())) |
                                                    <a class="btn btn-danger delete-expense-btn" data-id="{{ $txn->id }}">
                                    ...
                                    @endif
                                     <form method="POST" enctype="multipart/form-data" id="expenseForm">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select multiple name="items[][expense_item_id]" id="expense_item_id"   class="form-control">
                                                @foreach($expenseItems as $item)
                                                    <option value="{{ $item->id }}">{{ $item->expense_type_name }}</option>
                                                @endforeach
                                            </select>
                                            <div class="mt-4" id="generate_expen_tool" style="display:none;">
                                                        <button name="generate_expense" id="generate_expense" class="btn btn-danger blinking" type="button">Click Add to Proceed</button>
                                            </div>
                                            <div class="form-group" id="indiv_expense" style="display:none;">
                                         ...
                                         <div id="indiv_expense">
                                            
                                         </div>
                                        <div class="mb-3" id="total_amt">
          ...
          <!-- Modal -->
            ...
            <div class="modal-dialog modal-dialog-centered">
                ...
                <div class="modal-body text-center">
                    <img id="popupImage" src="" alt="Bill Image" class="img-fluid mb-3">
                    <div class="mb-3">
            ...
            <script src="{{ asset('assets/js/monthly_expense.js') }}"></script>
               <script src="{{ asset('assets/js/work_mode.js') }}"></script>
               <script>
                function Submit_MonthlyExpenseForm() {
                    const formData = new FormData($("#expenseForm")[0]);
                    const recordId = $("#recordId").val();
                    const url = recordId ? `/monthly_expense/${recordId}` : `{{ route('monthly_expense.store') }}`;
