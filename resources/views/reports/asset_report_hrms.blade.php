        ...
        <div class="content-wrapper">
            <div class="card shadow-sm">
                ...
                <br/>
                    ...
                    <div class="row form-row align-items-start" style="margin-left: 25% !important;">
                      <div class="form-group col-md-2">
...
<script type="text/javascript">
        ...
        $('#AssetsTable').DataTable({
                ...
                serverSide: true,
                  ajax: {
                        url: "{{ route('asset_report_action_hrms') }}",
                        type: "POST",
                        data: function(d) {
...
$('#assetReport').on('submit', function(e) {
   ...
   $('.download_tasks').on('click',function(){
    const item_type =  $('#search_item_type').val();
    const item_category=$('#search_item_category').val();
    const brand = $('#search_brand').val();
    const user_id = $('#search_user_id').val();

    const params = new URLSearchParams();

    if (item_type) params.append("item_type", item_type);
    if (item_category) params.append("item_category", item_category);
    if (brand) params.append("brand", brand);
    if (user_id) params.append("user_id", user_id);

    // ✅ append checkboxes as array
    $('#search_configure_attribute input.roleCheckbox:checked').each(function() {
        params.append("search_configure_attribute[]", $(this).val());
    });

    const downloadUrl = `/asset_report_export_hrms?${params.toString()}`;
   // alert(downloadUrl);
