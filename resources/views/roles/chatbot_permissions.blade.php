      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="card">
            ...
            @if (session('success'))
                ...
                <form action="{{route('update_bot_permission',$role->id)}}" method="post" id="myform">
                    @csrf

                    <div class="mb-3 row">
                        <label for="role_name" class="col-md-4 col-form-label text-md-end text-start">Role Name</label>
                        <div class="col-md-3">
                          <input type="text" class="form-control @error('role_name') is-invalid @enderror" id="role_name" name="role_name" value="{{$role->role_name}}" readonly>
                            @error('role_name')
                        ...
                        <thead>
                            ...
                            <tbody>
                                @foreach ($categories as $category)
                                    ...
                                    @foreach ($category->bot_children as $subCategory) 
                                    <tr>
                                        <td>{{ $subCategory->bot_name }}</td>
                                        @foreach ($permissions as $permission)
                                        @if ($loop->first)
                                            <td>
                                                <input type="checkbox" 
