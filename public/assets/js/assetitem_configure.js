        ...
        $('#item_type,#search_item_type').change(function(){
        ...
        $('#item_category,#search_item_category').change(function(){  
            if($(this).val() === 'others'){
            ...
            if($('#item_type').val() === 'asset'){
                GenerateConfiguration( $(this).val());
            } else {
                 $('#config_details_container').hide();
            }
            if($('#search_item_type').val() === 'asset'){
                let itemId = $(this).val(); // define itemId from the changed select
                $.get('/manage_configuration/' + itemId, function (config_response) {
                    if (config_response.load_data && config_response.load_data.length > 0) {
                        let options = "";
                        $.each(config_response.load_data, function(index, item) {
                            options += `<label class="checkbox-item">
                                        <input type="checkbox" class="roleCheckbox" name="search_configure[]" value="${item.attribute_id}">
                                        ${item.attribute_name}
                                        </label>`;
                        });
                        $('#search_configure').html(options);
                    }
                });
                $('#asset_configuration').show();
                
                $('.base_button').hide(); // keep hidden when asset
            } else {
                $('#asset_configuration').hide();
                $('#asset_config_features').hide();
                $('.base_button').show(); // show filter button otherwise
            }
        });
        ...
        $(document).on('change', 'input[name="search_configure[]"]', function() {
            // Collect all checked values into an array
            let selectedConfigs = $('input[name="search_configure[]"]:checked')
                                    .map(function() { return $(this).val(); })
                                    .get();

            console.log("Selected configuration IDs:", selectedConfigs);
            $('#asset_config_features').show();
            // Send the array to your backend
            $.ajax({
                url: '/manage_config_feature',
                type: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    config_ids: selectedConfigs
                },
                success: function(config_response) {
                    if (config_response.load_data && config_response.load_data.length > 0) {
                        let options = "";
                        $.each(config_response.load_data, function(index, item) {
                            options += `<label class="checkbox-item">
                                        <input type="checkbox" class="roleCheckbox" name="search_configure_attribute[]" value="${item.option_id}">
                                        ${item.option_name}
                                        </label>`;
                        });
                        $('#search_configure_attribute').html(options);
                    }
                }
            });
        });
...
// Add and Edit Modal Handling
        ...
        $(document).off('click', '.editButton').on('click', '.editButton', function () {
            ...
            $.get('/manage_items_configure/' + id + '/edit', function (data) {
                $('#AssetForm').trigger('reset');
                ...
                $('#item_type').val(data.item_type);
                 $('#config_details_container').hide();
                  if($('#item_type').val() === 'asset'){
                    GenerateConfiguration(data.item_category,data.id);
                  }
...
$.validator.addMethod("dateDMY", function(value, element) {
    return this.optional(element) || /^\d{2}-\d{2}-\d{4}$/.test(value);
        ...
        $("#AssetForm").validate({
            ...
            submitHandler: function (form) {
                ...
                const url = $('#recordId').val() ? '/manage_items_configure/' + $('#recordId').val() 
                                                : 'manage_items_configure/'; 
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            const url = '/manage_items_configure/' + itemId;
        ...
        $('#asset_status').change(function(){
            ...
            if(selectedval=='damaged' || selectedval=='retired'){
                $.get('/assets_manage/' + damagedId + '/replace_inventory', function (response) {
                    if(response.assigned==true  && response.available_items.length!=0){
                        $('#new_item').show();
    ...
    function GenerateConfiguration(assetTypeid,itemId){
        $.get('/manage_items_configure/' + assetTypeid+'/'+itemId, function (config_response) {
                                let html = '';
                                 html += 'Configuration Details:<hr/>';
                                config_response.forEach(function(attr) {
                                    html += '<div  class=" row mb-3"> <div class="col-md-3">';
                                    html += '<strong>' + attr.attribute_name + '</strong> </div>';
                                    html += '<div class="col-md-8"><div class="row">';

                                    attr.options.forEach(function(opt) {
                                        html += '<div class="col-md-4"><label style="margin-right:10px;">';
                                        html += '<input type="radio" name="attributes[' + attr.attribute_id + ']" value="' + opt.option_id + '"';
                                        if(opt.is_selected==true){
                                            html += ' checked'; 
                                        }
                                        html +='>';
                                        html += opt.option_name;
                                        html += '</label></div>';
                                    });

                                    html += '</div></div></div>';
                                });
                                $('#config_container').html(html); // inject into your div
                        });
