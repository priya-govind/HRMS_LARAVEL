...
// Only allow slots that end at or before current time
//const isDisabled = toDate > currentTime || isWithinAnyPermission(from_time, to_time);
const isDisabled = new Date(`1970-01-01T${from_time}`) < checkInDate || isWithinAnyPermission(from_time, to_time);
const slotClass = isDisabled ? 'timeslot disabled-slot' : 'timeslot';
...
$('#formContainer').on('change keyup', 'input, textarea, select', function () {
  ...
  // Check with backend if slot is blocked
  $.post('/check-permission-slot', {
    from_time,
    to_time,
    ...
    if (response.blocked) {
      // Disable the slot
      $(`#slot${index} input, #slot${index} textarea, #slot${index} select`).prop('disabled', true);
      console.log(`Slot ${index + 1} blocked due to permission.`);
      return;
    }
...
function isWithinAnyPermission(from, to) {
  if (!permissionRanges || !Array.isArray(permissionRanges)) return false;

  const slotFrom = new Date(`1970-01-01T${from}`);
  const slotTo = new Date(`1970-01-01T${to}`);

  return permissionRanges.some(range => {
    const permFrom = new Date(`1970-01-01T${range.from_time}`);
    const permTo = new Date(`1970-01-01T${range.to_time}`);
    return slotFrom < permTo && slotTo > permFrom;
  });
}
function submitForm() {
