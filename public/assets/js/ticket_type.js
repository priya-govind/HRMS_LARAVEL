...
$('#addButton').click(function () {
    ...
    myModal.show();
    $('#dataModalLabel').text('Add New Ticket Type');
...
$(document).off('click', '.editButton').on('click', '.editButton', function () {
    ...
    $.get('/ticket_types/' + id + '/edit', function (data) {
        $('#dataModalLabel').text('Edit Ticket Type');
        $('#recordId').val(data.id);
        $('#ticket_type').val(data.ticket_type);
        $('input[name="ticket_type_active"][value="' + data.ticket_type_active + '"]').prop('checked', true);
    });
});
// Handle Form Submission with Validation
            ...
            $("#dataForm").validate({
                ...
                messages: {
                    ticket_type: {
                        ...
                        pattern: "Special characters are not allowed in the Ticket Type name.",
                    },
                    ticket_type_active: {
                        required: "Please select a Ticket Type status.",
                    },
                },
                ...
                submitHandler: function (form) {
                    ...
                    const url = isEdit ? '/ticket_types/' + $('#recordId').val() : '/ticket_types';
        ...
        const url = '/ticket_types/' + id + '/status_change';
        // Confirm dialog
        $.ajax({
            url: url,
           // type: 'POST',
