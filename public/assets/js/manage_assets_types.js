...
// Add and Edit Modal Handling
        ...
        $(document).off('click', '.editButton').on('click', '.editButton', function () {
            ...
            $.get('/edit_assets_types/' + id , function (data) {
                $('#dataModalLabel').text('Edit Asset Type Name');
                $('#recordId').val(data.asset_type_info.id);
                $('#asset_type_name').val(data.asset_type_info.asset_type_name);
                $('input[name="asset_type_status"][value="' + data.asset_type_info.asset_type_status + '"]').prop('checked', true);
                const AssetConfigure = Array.isArray(data.asset_config) ? data.asset_config : [];
                            // Pre-check configuration alerted checkboxes
                            AssetConfigure.forEach(attributeId => {
                                $(`input[name="configure[]"][value="${attributeId}"]`).prop('checked', true);
                            });
            });
        });
        ...
        $("#dataForm").validate({
            ...
            submitHandler: function (form) {
                ...
                const url = $('#recordId').val() ? '/update_assets_types/'  
                                                : '/store_assets_types'; 
        ...
        $(document).on('click', '.delete-btn', function () {
            ...
            const url = '/destroy_assets_types/' + itemId ;
