...
class TransactionController extends Controller
{
    ...
    {
       // dd($request);
        $balance = Transaction::latest()->where('is_deleted', false)->pluck('available_amt')->first();
    ...
    public function store(Request $request)
    {
       $validator = Validator::make($request->all(), [
            ...
            if ($validator->fails()) {
                ...
                $amt = Transaction::latest()->where('is_deleted', false)->first();
                $last_avail_balance = $amt ? $amt->available_amt : 0;
                if($request->transaction_type=='credit'){
                    $present_balance=$last_avail_balance+$request->amount;
                } else if($last_avail_balance < $request->amount){
                    return response()->json(['error' => "Insufficient balance for this debit transaction. Available balance: $last_avail_balance"], 400);            
                    //  return redirect()->back()->withErrors("Insufficient balance for this debit transaction. Available balance: $last_avail_balance")->withInput();
                }else {
                    $present_balance=$last_avail_balance-$request->amount;
                }
                $imageName =''; 
...
public function update(Request $request, $id)
{
    ...
    if ($transaction->transaction_type === 'debit' || $transaction->last_entry === 1) {
        $imgrules=$transaction->bill_refer ? "required|file|mimes:jpg,jpeg,png,pdf" : 'nullable|file|mimes:jpg,jpeg,png,pdf';
         $request->validate([
        ...
        if ($transaction->transaction_type === 'debit'){
             $adjustedBalance = $last->available_amt + $transaction->amount;
                ...
                // Upload bill
                $imageName = $transaction->bill_refer;
                if ($request->hasFile('bill_refer')) {
                    $file = $request->file('bill_refer');
                    $imageName = time() . '.' . $file->extension();
                    $file->move(public_path('bills'), $imageName);
